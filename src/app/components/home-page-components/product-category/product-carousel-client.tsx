...
export default function ProductCarouselClient({
  ...
  const getOffsetForIndex = useCallback(
    (index: number): number => {
      if (isMobile || !trackRef.current || !trackRef.current.children[index])
        return 0;
  ...
  return (
    <section className="w-full flex justify-center bg-white pb-8 sm:pb-12 md:pb-16">
      <div className="w-full max-w-[2000px] overflow-hidden relative">
        <h2
          className={`text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-6 text-[#2b2b2b] ${avertaBlack.className}`}
        >
          {title}
        </h2>
        ...
        <div className="relative px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16">
          ...
          <button
            className="absolute left-[39px] top-1/2 -translate-y-1/2 bg-black text-white p-2 md:p-3 shadow-lg hover:bg-gray-800 transition-all duration-300 z-10 hidden md:flex"
            onClick={prevSlide}
          ...
          <button
            className="absolute right-[39px] top-1/2 -translate-y-1/2 bg-black text-white p-2 md:p-3 shadow-lg hover:bg-gray-800 transition-all duration-300 z-10 hidden md:flex"
            onClick={nextSlide}
