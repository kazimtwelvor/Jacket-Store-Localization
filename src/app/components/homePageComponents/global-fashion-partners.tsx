...
const GlobalFashionPartners = () => {
  ...
  const handleNext = () => setCurrentPartner((prev) => (prev + 1) % partners.length)
  const handlePrev = () => setCurrentPartner((prev) => (prev - 1 + partners.length) % partners.length)

  // Auto-advance testimonials and mobile carousel
  useEffect(() => {
    if (isAutoPlaying) {
    ...
    <section className="bg-gradient-to-br from-white to-gray-50 py-12 md:py-16 lg:py-20 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="relative mb-6 md:mb-8">
          ...
          {/* Desktop: All logos with highlighting */}
                ...
                >
                  <img 
                    ...
                    className={`h-12 md:h-16 object-contain transition-all duration-500 ${
                      index === currentPartner ? 'scale-110 opacity-100' : ''
                    }`}
                    style={{ 
        ...
        {/* Desktop: Testimonials and pagination */}
                ...
                {partners.map((_, index) => (
                  <motion.button
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      index === currentPartner ? 'bg-[#2b2b2b] w-8' : 'bg-gray-300'
                    }`}
