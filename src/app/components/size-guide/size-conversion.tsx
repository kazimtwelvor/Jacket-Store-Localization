...
import { SizeConversionData } from "@/src/app/(routes)/[country]/size-guide/data/size-guide-data-by-country"

interface SizeConversionProps {
  sizeConversionData?: SizeConversionData
}

export default function SizeConversion({ sizeConversionData }: SizeConversionProps) {
  ...
  const categories = sizeConversionData?.categories || [
    { id: "clothing", label: "Clothing" },
    { id: "shoes", label: "Shoes" },
    { id: "accessories", label: "Accessories" },
  ]

  const clothingSizeConversion = sizeConversionData?.clothingSizeConversion || [
  ...
  const shoesSizeConversion = sizeConversionData?.shoesSizeConversion || [
  ...
  const accessoriesSizeConversion = sizeConversionData?.accessoriesSizeConversion || [
  ...
  if (!isMounted) {
    return (
      <section className="space-y-8">
        <div className="mb-8">
          <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-3">{sizeConversionData?.title || "Size Conversion"}</h2>
          <p className="text-muted-foreground max-w-3xl">
            {sizeConversionData?.description || "Convert between different international sizing systems to find your perfect fit."}
          </p>
        </div>
  ...
  return (
    <motion.div
      ...
      className="space-y-8"
    >
      <h2 className="text-2xl md:text-3xl font-bold tracking-tight mb-3 text-[#2b2b2b]">{sizeConversionData?.title || "Size Conversion"}</h2>
      ...
      <div className="bg-white rounded-lg border border-[2b2b2b] p-6 mt-8">
        <h3 className="text-xl font-semibold mb-4 text-[#2b2b2b]">Size Conversion Tips</h3>
        <ul className="space-y-2">
          {(sizeConversionData?.tips || [
            "Sizes can vary between brands, so always check the specific brand's size chart when available.",
            "When in doubt, go with your measurements rather than your usual size.",
