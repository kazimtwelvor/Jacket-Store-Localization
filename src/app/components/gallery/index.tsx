...
import { ChevronLeft, ChevronRight, X, ZoomIn, ZoomOut, RotateCcw, Heart } from "lucide-react"
...
const Gallery: React.FC<GalleryProps> = ({ images = [], onAddToCart, onAddToWishlist, isInWishlist = false }) => {
  ...
  const prevViewerImage = () => {
    setViewerIndex((prev) => (prev - 1 + sortedImages.length) % sortedImages.length)
    setZoomLevel(1)
    setImagePosition({ x: 0, y: 0 })
  }
  ...
  const handleViewerTouchEnd = (e: React.TouchEvent) => {
    if (e.changedTouches.length === 1 && zoomLevel <= 1 && Math.abs(viewerTouchStart - viewerTouchEnd) > 100) {
          ...
          >
            <div className="relative flex-1 flex items-center justify-center pt-0 sm:pt-20 pb-12 sm:pb-16">
              ...
              <div className="absolute top-6 left-6 z-10 hidden sm:block">
                <h1 className="text-red-600 text-2xl font-bold tracking-wide" style={{ fontFamily: 'AvertaPE, sans-serif' }}>
                  FINEYST
                </h1>
              </div>
              ...
              >
                ...
                >
                  <img
                    src={sortedImages[viewerIndex]?.image.url || "/placeholder.svg"}
                    alt={sortedImages[viewerIndex]?.image.altText || "Product image"}
                    className="block"
                    style={{ 
                      objectFit: 'contain',
                      maxWidth: '90vw',
                  ...
                  {sortedImages.map((image, index) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-lg border-2 overflow-hidden transition-all shadow-lg ${
                        index === viewerIndex ? 'border-red-600 opacity-100 scale-110' : 'border-white/30 opacity-70 hover:opacity-100 hover:scale-105'
                      }`}
                    >
                      <img
  ...
  return (
    <div className="w-full">
      ...
      {typeof window !== 'undefined' && createPortal(imageViewerModal, document.body)}

      {/* Desktop view - scrollable 2 column grid */}
      <div className="hidden lg:grid grid-cols-2 gap-0" style={{ paddingTop: '26px' }}>
...
<div className="lg:hidden relative pt-0 pb-0 product-gallery ml-0 sm:ml-0">
  ...
  <div 
    ...
    style={{ marginLeft: 'calc(-50vw + 50%)', width: '100vw', aspectRatio: '4/6' }}
  ...
  >
    ...
    <div className="absolute top-3 left-3 bg-white/60 text-black px-3 py-2  text-xs font-medium backdrop-blur-sm">
      {currentIndex + 1}/{sortedImages.length}
        ...
        <div className="absolute bottom-01 right-3 flex items-center gap-1 z-[9999]" style={{ transform: 'translateY(-50%)' }}>
          {/* Wishlist Button */}
          <button
            onClick={(e) => {
              e.stopPropagation()
