...
export function CapsuleNav() {
  ...
  useEffect(() => {
    ...
    if (isCategoryPage || isShopPage) {
      window.addEventListener('filterBarSticky', handleFilterBarSticky as EventListener)
    }
    ...
    return () => {
      ...
      if (isCategoryPage || isShopPage) {
        window.removeEventListener('filterBarsticky', handleFilterBarSticky as EventListener)
      }
    }
                ...
                <div key={item.id} className="relative mr-1">
                  <Button
                    ...
                    className={cn(
                      ...
                      item.id === activeItem && "bg-[#2b2b2b] text-white hover:bg-[#2b2b2b]/90",
                    )}
  ...
  return (
    <div className={cn(
      ...
      ((isCategoryPage || isShopPage) && isMobileView && isExpanded) ? "w-[95%]" : "w-[70%]",
      // Base positioning: fixed on mobile product pages, relative otherwise.
      (isProductPage && isMobileView) ? "fixed top-[60px] left-0 right-0 z-40 px-4" : "relative",
