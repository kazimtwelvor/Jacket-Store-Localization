...
import { Filter, ChevronDown, ArrowUpDown, HelpCircle, Heart, X, ChevronLeft, ChevronRight } from "lucide-react"
...
import JacketCategories from "@/components/category/JacketCategories"
import WeThinkYouWillLove from "@/components/category/WeThinkYouWillLove"
...
import MobileAddToCartModal from "@/components/modals/MobileAddToCartModal"
...
// Helper function to get local image URL
...
const getProductSlug = (product: Product): string => {
  ...
  if (product.name) {
    const baseSlug = product.name
      ...
      .replace(/-+/g, "-")
      .trim()
    // Add part of the ID to make it unique
...
const CategoryPageClient: React.FC<CategoryPageClientProps> = ({ category, products, slug, allCategories, keywordCategories = [], isKeywordCategory = false }) => {
  ...
  const [mounted, setMounted] = useState(false);
  // Add a guard to prevent crashes if the category prop isn't available,
  // which can happen during client-side route transitions in Next.js.
  if (!category) {
    // Render nothing or a loading spinner while waiting for props.
    return null;
  }
  // Get current sort and filters from URL
  ...
  // Mock data for sizes and colors - replace with actual data fetching
  ...
  useEffect(() => {
    // Set up intersection observer for real implementation
  ...
  // Close sort dropdown and color popup when clicking outside
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Close color popup if clicking outside
  ...
  // Perform all layout measurements once, after the initial render.
  useLayoutEffect(() => {
  ...
  // Handle sticky filter bar state using robust, pre-calculated positions
  useEffect(() => {
    // Exit if measurements haven't been taken yet.
  ...
  }, [layoutMetrics]); // Reruns only when layoutMetrics are calculated.
  ...
  const handleSizeSelect = (productId: string, size: string) => {
    // Find product in either the main list or recently viewed list
  ...
  const addToRecentlyViewed = (product: Product) => {
    setRecentlyViewed((prev) => {
      ...
      // Save to localStorage for the product page to read
      if (typeof window !== 'undefined') {
  ...
  // Initialize current products with pagination
  useEffect(() => {
  ...
  // Parse categoryContent if it's a string
  ...
  // Create current category object
  ...
  // Get categories for the JacketCategories component
  ...
  if (parsedCategoryContent?.otherCategories && parsedCategoryContent.otherCategories.length > 0) {
    ...
    // Fallback to default categories from allCategories
  ...
  // Prepare structured data for schema
    ...
    <div className="bg-white overflow-x-hidden">
      ...
      <div className="container mx-auto pt-24 sm:pt-22 md:pt-20 lg:pt-20 xl:pt-20 pb-0 sm:pb-2 md:pb-6 lg:pb-6 xl:pb-6 text-center">
        <JacketCategories
          ...
          onCategoryClick={(categorySlug) => router.push(`/category/${categorySlug}`)}
      ...
      <div className="mx-auto w-full px-2 sm:px-6 md:px-6 lg:px-8 xl:px-8 py-3 sm:py-3 md:py-6 lg:py-6 xl:py-6">
              ...
              {currentProducts.map((product, index) => (
                ...
                >
                  ...
                  >
                    ...
                    {/* Quick Shop Size Selection - Shows on hover (desktop only) */}
                    <AnimatePresence>
        ...
        <div id="lower-content-section">
          ...
          {/* Add SEO Section at the bottom of the page */}
          <CategorySEOSection
            category={{
              ...category,
...
2.15a1 1 0 0 0 .99-.84l.58-3.47a2 2 0 0 0-1.34-2.23z"
                  ...
                  <motion.div
                    key={index}
                    className="border rounded-md p-4 flex flex-col" // Changed to flex-col
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    {/* Wrapper for content that should take up space and push button down */}
                    <div className="flex flex-col flex-grow items-center text-center"> {/* Added flex-grow */}
                      <motion.div className="mb-2" whileHover={{ rotate: 5 }}>
                        {category.icon}
                      </motion.div>
                      <h3 className="font-bold mb-2">{category.title}</h3>
                      <p className="text-sm text-gray-600 mb-4">{category.description}</p> {/* Removed text-center as parent is now flex-col items-center */}
                    </div>
                    {/* Button aligned to the bottom */}
                    <motion.button
                      className="bg-black text-white hover:bg-gray-800 w-full py-2 rounded flex items-center justify-center flex-shrink-0" // Added flex-shrink-0
