...
export default async function BlogPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const renderStepContent = (step: any) => {
      ...
      if (step.cardTitles && step.cardDescriptions) {
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {step.cardTitles.map((title: string, index: number) => (
              <section key={index} className="flex flex-col items-center text-center">
                {step.images[index] && (
                  <section className="relative w-48 h-48 rounded-full overflow-hidden border-4 border-white shadow-lg mb-6">
        ...
        return (
          <section className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {step.images.filter(Boolean).map((image: string, index: number) => (
              <section key={index} className="relative aspect-square rounded-lg overflow-hidden shadow-md">
                <Image src={image.replace('\n', '')} alt={step.imageLabels?.[index] || `Image ${index + 1}`} fill className="object-cover" />
                {step.imageLabels?.[index] && (
                  <section className="absolute bottom-0 left-0 right-0 bg-black/70 text-white p-2 text-sm">
                    {step.imageLabels[index]}
                  </section>
                )}
              </section>
        ...
        return (
          <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {step.images.filter(Boolean).map((image: string, index: number) => (
              <section key={index} className="relative h-[200px] rounded-lg overflow-hidden shadow-md">
      ...
      if (step.metrics.before && step.metrics.after) {
        return (
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section className="bg-red-50 p-6 rounded-lg border-t-4 border-red-500">
            ...
            <section className="bg-green-50 p-6 rounded-lg border-t-4 border-green-500">
        ...
        return (
          <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {step.metrics.titles.map((title: string, index: number) => (
              <section key={index} className="bg-white p-6 rounded-lg shadow-md border-t-4 border-[#0A2463] text-center">
                <h3 className="text-lg font-bold text-[#0A2463] mb-2">{title}</h3>
                <section className="text-3xl font-bold text-[#B01E23]">{step.metrics.values[index]}</section>
              </section>
            ))}
          </section>
        )
    ...
    if (step.timelineItems) {
      return (
        <section className="relative">
          <section className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-[#0A2463] to-[#B01E23]"></section>
          {step.timelineItems.map((item: any, index: number) => (
            <section key={index} className="relative mb-16">
              <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <section className={index % 2 === 0 ? "md:text-right" : "md:order-2 md:text-left"}>
                  <section className={`bg-white p-6 rounded-lg shadow-md border-t-4 border-[#0A2463] ${index % 2 === 0 ? "md:ml-auto" : "md:mr-auto"}`}>
                    <h3 className="text-lg font-bold text-[#0A2463] mb-2">{item.title}</h3>
                    <p className="text-gray-700">{item.description}</p>
                  </section>
                </section>
                <section className={`relative ${index % 2 === 0 ? "" : "md:order-1"}`}>
                  <section className="h-[200px] relative rounded-lg overflow-hidden shadow-md">
                    {step.images?.[index] ? (
                      <Image src={step.images[index].replace('\n', '')} alt={item.title} fill className="object-cover" />
                    ) : (
                      <section className="bg-gray-200 w-full h-full flex items-center justify-center">
                        <section className="text-gray-400">Image placeholder</section>
                      </section>
                    )}
                  </section>
                </section>
              </section>
              <section className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-[#0A2463] border-4 border-white"></section>
            </section>
          ))}
        </section>
      )
    ...
    return (
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        {step.image && (
          <section className="relative h-[300px] rounded-lg overflow-hidden shadow-md">
  ...
  return (
    <section className="bg-white min-h-screen pt-24">
      <section className="fixed top-0 left-0 h-1 bg-gradient-to-r from-[#B01E23] to-[#0A2463] w-0 z-[9999] transition-all duration-200" id="reading-progress-bar"></section>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <section className="mb-10">
          <section className="flex items-center gap-2 mb-4">
            <section className="w-1 h-8 bg-[#B01E23] rounded-full"></section>
          ...
          <section className="space-y-12">
            {guideContent.steps.filter((step: any) => step.isActive).map((step: any, index: number) => (
              <section key={step.id} className="animate-fade-in my-16">
        ...
        <section className="animate-fade-in my-16">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section className="bg-white p-6 rounded-lg shadow-md border-t-4 border-[#0A2463]">
            ...
            <section className="bg-white p-6 rounded-lg shadow-md border-t-4 border-[#B01E23]">
