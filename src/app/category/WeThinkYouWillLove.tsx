...
const WeThinkYouWillLove: React.FC<WeThinkYouWillLoveProps> = ({
  products,
  hoveredProduct,
  setHoveredProduct,
  selectedSizes,
  ...
  const displayedProducts = currentProducts.slice(0, 8)
                  ...
                  <div className="mt-1">
                    {product.salePrice && Number(product.salePrice) > 0 ? (
                      <div className="flex items-center gap-1">
                        <span className="text-sm font-bold line-through text-gray-500">${product.price}</span>
                        <span className="text-lg font-bold text-[#B61f28]">${product.salePrice}</span>
                      </div>
                    ) : (
                      <span className="text-lg font-bold text-[#B61f28]">${product.price}</span>
                    )}
                  </div>
                  <div className="mt-2">
                    <div 
              ...
              {displayedProducts.map((product, index) => (
                ...
                >
                  <div className="relative overflow-hidden bg-gray-100 w-full aspect-[3/5]">
                    ...
                    <AnimatePresence>
                      {hoveredProduct === `love-${product.id}-${index}` && ((product as any).sizeDetails || (product as any).sizes) && ((product as any).sizeDetails || (product as any).sizes).length > 0 && (
                        <motion.div
                      ...
                      >
                        ...
                        {((product as any).colorDetails || (product as any).colors || []).length > 1 && (
                          <>
                            <button 
                              ref={(el) => {
                                if (el) colorTriggerRefs.current[`love-${product.id}-${index}`] = el
                              }}
                              data-color-trigger
                              onClick={(e) => {
                                e.stopPropagation()
...
export default WeThinkYouWillLove