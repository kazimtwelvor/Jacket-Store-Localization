...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // First replace special characters with hyphens
    let sanitizedFilename = file.name.replace(/[\s_&%#?]+/g, '-')
    // Then remove any remaining invalid characters except letters, numbers, dots, and hyphens
    sanitizedFilename = sanitizedFilename.replace(/[^a-zA-Z0-9.-]/g, '')
